pipeline {
    ...
    environment {
        ...
        DOCKER_HUB_USERNAME = 'saleemkhandev' // <--- UPDATED DOCKER HUB USERNAME
        ...
        // You will need to create this in Jenkins as a "Secret Text" credential.
    ...
    stages {
        stage('Checkout Code') {
            steps {
                // Use your GitHub PAT credential for cloning if the repo is private or you have rate limit issues
        ...
        stage('Login to Docker Hub') {
            steps {
                // Assuming DOCKER_HUB_PASSWORD is a Secret Text credential in Jenkins
                // with ID 'dockerhub-password-credential'
        ...
        stage('Deploy to EC2') {
            agent {
                // SSH agent for sending commands to the target server
                // The SSH_KEY_ID must match the credential ID in Jenkins
